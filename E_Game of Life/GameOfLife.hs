...
nextGeneration :: Options -> World -> World
nextGeneration o w = S.union (S.filter (\x -> neighborCountIn x w == 2 || neighborCountIn x w == 3) w)
                           ((S.filter (\x -> neighborCountIn x w == 3)) (deadCells o w))


-- | Helper functions
deadCells :: Options -> World -> World
deadCells o w = S.difference (fullWorld o) w

fullWorld :: Options -> World
fullWorld o@(Options _ _ _ _ (xs,ys,xe,ye) _ _) = fromList
                                                  ((,) <$> [xs .. xe] <*> [ys .. ye])
--NOTES: I added Options as an input in nextGeneration so I could generate a set of all points 
